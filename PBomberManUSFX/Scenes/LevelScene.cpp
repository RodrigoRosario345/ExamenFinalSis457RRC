...
void LevelScene::spawnEnemy(GameTexture texture, AIType type, const int positionX, const int positionY)
{
    ...
    if (EnemyClonado != nullptr) {
        EnemyClonado->setVelocidad(0.0055f);
        cout << "Se ha clonado satisfactoriamente el enemigo 1" << endl;
        addObject(EnemyClonado);
        enemies.push_back(EnemyClonado);
    }
...
//-----------------------------------------------------
void LevelScene::spawnEnemy2(GameTexture texture, AIType type, const int positionX, const int positionY)
{
    //FactoryEnemy::initialize();
    //std::shared_ptr<EnemigoFantasma>  EnemyClonado;
    //std::shared_ptr<EnemigoFantasma> enemy2;

    //enemy2 = dynamic_pointer_cast<Enemy>(factory->CreateEnemy(type, positionX, positionY));
    //EnemyClonado = enemy2->clone();
    //if (EnemyClonado != nullptr) {
    //    //EnemyClonado->setAIType(type);
    //    cout << "Se ha clonado satisfactoriamente el enemigo 1" << endl;
    //    addObject(EnemyClonado);
    //    enemiesFantasma.push_back(EnemyClonado);
    //}



    auto enemy2 = std::make_shared<EnemigoFantasma>(gameManager->getAssetManager()->getTexture(texture), gameManager->getRenderer());
    //auto enemy2 = std::make_shared<ClasicoEnemy>(gameManager->getAssetManager()->getTexture(texture), gameManager->getRenderer());
...
/////////////////////////--------------------------------------------------aca clonavamos al enemigo 
// Enemy* EnemyClonado = nullptr;
// FactoryEnemy::initialize();
//void LevelScene::spawnEnemy(GameTexture texture, AIType type, const int positionX, const int positionY)
//{
//    std::shared_ptr<Enemy> enemy;
...
void LevelScene::generateEnemies2()
{
    // we need enemy in random tile
    const auto seed = std::chrono::high_resolution_clock::now().time_since_epoch().count();
    auto randCount = std::bind(std::uniform_int_distribution<int>(minEnemiesOnLevel, maxEnemiesOnLevel),
...
void LevelScene::updatePlayerCollision()
{
    ...
    if(door != nullptr)
    {
        ...
        {
            // check win condition
            if(!isGameOver && enemies.size() && enemiesFantasma.size() == 0)
            {
                gameOver();
                isWin = true;
            ...
            if (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - m_start).count() >= 10000) {
                ...
                if (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - m_start).count() >= 5000) {
                    ...
                    if (isCollisionDetected(enemy->getRect(), collisionObject.second->getRect()))
                    {
                        ...
                        cout << " estas en el enemigo respetuoso " << endl;
                        //m_start = std::chrono::steady_clock::now();
                    }
...
void LevelScene::updateEnemies2Collision()
{
    // iterate enemies for collision
    for (const auto& enemy2 : enemiesFantasma)
    {
        // iterate drawables for collision
        for (const auto& collisionObject : collisions)
        {
            // check for block collision
            if (isCollisionDetected(enemy2->getRect(), collisionObject.second->getRect()))
            {
                // stop moving on collision detection
                enemy2->setMoving(false);
                enemy2->revertLastMove();
            }
        }
        // check for bomb collision
        if (bomb && isCollisionDetected(enemy2->getRect(), bomb->getRect()))
        {
            // stop moving on collision detection
            enemy2->setMoving(false);
            enemy2->revertLastMove();
        ...
        if (player != nullptr)
        {
            ...
            playerRect.w = static_cast<int>(playerRect.w * 0.2);
            playerRect.h = static_cast<int>(playerRect.h * 0.2);
            if (isCollisionDetected(playerRect, enemy2->getRect()))
            {
                // player killed by enemy
                removeObject(player);
                player = nullptr;
        ...
        {
            // can attack?
            if (!enemy2->isMovingToCell() && enemy2->canAttack())
            {
                // check for attack radius
                if (abs(player->getPositionX() + player->getWidth() / 2 - enemy2->getPositionX() -
                    enemy2->getWidth() / 2) < enemy2->getAttackRadius() &&
                    abs(player->getPositionY() + player->getHeight() / 2 - enemy2->getPositionY() -
                        enemy2->getHeight() / 2) < enemy2->getAttackRadius())
                {
                    // start follow to player
                    followToPlayer(enemy2.get());
                }
            }
        }
    }
...
void LevelScene::updateBangs2Collision()
{
    // check for bang collision
    for (const auto& bang : bangs)
    {
        // check bricks
        auto itCollision2 = collisions.begin();
        while (itCollision2 != collisions.end())
        {
            if ((*itCollision2).first == GameTile::Brick)
            {
                auto brick = (*itCollision2).second;
                if (isCollisionDetected(brick->getRect(), bang->getRect()))
                {
                    destroyBrick(brick);
                    // remove brick from collision array
                    itCollision2 = collisions.erase(itCollision2);
                    continue;
                }
            }
            ++itCollision2;
        ...
        // check enemies
        auto itEnemies2 = enemiesFantasma.begin();
        while (itEnemies2 != enemiesFantasma.end())
        {
            SDL_Rect enemyRect = (*itEnemies2)->getRect();
            enemyRect.w = static_cast<int>(enemyRect.w * 0.2);
            enemyRect.h = static_cast<int>(enemyRect.h * 0.2);
            if (isCollisionDetected(enemyRect, bang->getRect()))
            {
                removeObject(*itEnemies2);
                itEnemies2 = enemiesFantasma.erase(itEnemies2);
